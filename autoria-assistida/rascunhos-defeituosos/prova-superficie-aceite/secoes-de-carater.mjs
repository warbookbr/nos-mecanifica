...
export function construirPeleDianteira() {
  ...
  /* Alvo regional P0: a abertura precisa coincidir na vista lateral com o
     arco de centro y=340 mm e raio 385 mm; não é um círculo decorativo. */
  const centroDaRoda = { y: 340, z: 1325 };
  const raioInterno = 385;
  const raioExterno = 425;
  /* Quarenta trechos preservam a curvatura lida no recorte, sem mudar centro,
     raio ou topologia da abertura. Dez deixavam quinas visíveis ao crítico. */
  const segmentos = 40;
  const arcoInterno = [];
  const arcoExterno = [];
  for (let i = 0; i <= segmentos; i += 1) {
    ...
    /* L14 fixa também a posição transversal do topo. A largura do lábio é
       mantida por dois loops próximos, sem deslocar a abertura do alvo. */
    V.set(interno, ponto(raioInterno, 830));
    V.set(externo, ponto(raioExterno, 860));
  ...
  /* Uma faixa intermediária divide a antiga ponte alta em duas transições de
     quads. A borda do arco deixa de terminar em uma cunha na vista isométrica. */
  const trilhoIntermediario = trilhoDoOmbro.map((idTrilho, i) => {
    const alto = V.get(idTrilho); const baixo = V.get(arcoExterno[i]);
    const idIntermediario = proximoVertice + (segmentos + 1) * 3 + i;
    V.set(idIntermediario, alto.map((valor, eixo) => valor * 0.52 + baixo[eixo] * 0.48));
    return idIntermediario;
  });
  for (let i = 1; i <= segmentos; i += 1) {
    const primeiroId = proximoArco + segmentos + (i - 1) * 2;
    F.set(primeiroId, { id: primeiroId, parte: 'gradeLocalDoArco', vs: [trilhoDoOmbro[i - 1], trilhoDoOmbro[i], trilhoIntermediario[i], trilhoIntermediario[i - 1]] });
    F.set(primeiroId + 1, { id: primeiroId + 1, parte: 'transicaoDoArco', vs: [trilhoIntermediario[i - 1], trilhoIntermediario[i], arcoExterno[i], arcoExterno[i - 1]] });
  }
  ...
  const loopDoFarol = bordaExternaDoFarol.map((vertice, indice) => {
    const p = V.get(vertice);
    /* A faixa do farol começa depois dos dois loops e do trilho do arco. Assim
       aumentar a resolução do arco nunca sobrescreve seus vértices. */
    const idDoVertice = proximoVertice + (segmentos + 1) * 4 + 10 + indice;
    V.set(idDoVertice, p.map((v, eixo) => v + (centroDoFarol[eixo] - v) * 0.38));
    return idDoVertice;
  });
  const primeiroIdDoFarol = proximoArco + segmentos * 3;
  for (let i = 0; i < 4; i += 1) {
    const proximo = (i + 1) % 4;
