/* secoes-de-carater.test.mjs — fixa a contraevidência estrutural do gerador privado por seções. */
...
describe('prova R2 por seções de caráter', () => {
  ...
  it('gera somente quads e preserva as três regiões da mesma pele', () => {
    ...
    expect(pele.F.size).toBe(142);
    expect([...pele.F.values()].every((f) => f.vs.length === 4)).toBe(true);
    expect(new Set([...pele.F.values()].map((f) => f.parte))).toEqual(new Set(['capo', 'quebraDeOmbro', 'flanco', 'arcoDeRoda', 'gradeLocalDoArco', 'transicaoDoArco', 'gradeLocalDoFarol']));
  });

  it('deixa o centro do arco e o retalho do farol fora da pele', () => {
    const pele = construirPeleDianteira();
    expect(pele.aberturas.arcoDeRoda.loop).toHaveLength(41);
    expect(pele.aberturas.arcoDeRoda.centroDaRoda).toEqual({ y: 340, z: 1325 });
    expect(pele.aberturas.arcoDeRoda.raioInterno).toBe(385);
    expect(pele.V.get(pele.aberturas.arcoDeRoda.loop[20])).toEqual([830, 725, 1325]);
