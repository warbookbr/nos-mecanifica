...
export const FORMATO_JULGAMENTO_CRITICO = 'mecanifica.julgamento-critico@2';
const SHA = /^sha256:[a-f0-9]{64}$/;
const DECISOES = new Set(['primeira', 'segunda', 'empate', 'indeterminado']);
...
export function validarJulgamentoCritico(entrada) {
  ...
  const chaves = ['achados', 'apresentacao', 'assinaturaLote', 'assinaturaResposta', 'confianca', 'decisao', 'formato', 'hashPrompt', 'lote', 'modelo', 'provedor'];
  ...
  for (const chave of ['lote', 'apresentacao', 'provedor', 'modelo']) if (typeof entrada[chave] !== 'string' || !entrada[chave].trim()) falhar(`${chave} obrigatório.`);
