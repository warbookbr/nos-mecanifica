...
| UP-008 | `origem` estrutural em TODO gerador e portas nomeadas pelo autor | em prova | R4 adicionou origem a `chamferBox`, `esfera`, `cone`, `plano` e `inflate`, além de `publicarPorta`/`sel:{porta}`; pinça e suporte do freio usam `chamferBox` com 0 face sem identidade e o teste prova a porta após rotação. Falta consolidar a fixture não automotiva antes de chamar de provada | Todo gerador fica endereçável sem IDs de runtime; portas separam o nome funcional escolhido pelo autor do nome geométrico local da primitiva |
