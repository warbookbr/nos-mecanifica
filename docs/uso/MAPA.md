...
203 arquivos (código `.js .mjs .cjs .ts .tsx .html` + docs `.md`).
...
- `jardineira-integridade.test.ts` — jardineira-integridade.test.ts — a prova NÃO AUTOMOTIVA do contrato de autoria da Fundação de autoria v1 (O-6 `origem` universal + O-12 portas semânticas).
