...
- **Número tem que ser FINITO; ponto tem que ter 3 elementos** (2º achado
  ...
  antes da guarda, o estrago num TOPO era INVISÍVEL a todo gate: `lados: NaN`
...
| FEITO | `cone` | `id`, `raio`, `altura`, `lados` (mín 3) | FEITO (P1, só núcleo). `lados` aceita contagem ou `{desvio: medida}`. Anel da base em y=0 (b+0..b+lados−1), ápice em y=altura (b+lados). Faces: laterais b+j (triângulos pra fora) + tampa da base b+lados (mesmo winding do fundo do cilindro, normal −y). |
...
| FEITO | `furo` | `origemId` (OBRIGATÓRIO), `de` (origem estrutural que resolve para EXATAMENTE uma face — a ENTRADA), `centro` ou `centros` (pontos, discos ou círculos; projetados no plano), `raio` (PARAM > 0, padrão do passo), `lados?` (TOPO: número mín 3 ou `{desvio: medida}`), **exatamente uma** de `saida` (origem estrutural de UMA face — PASSANTE) ou profundidades (a `profundidade` PARAM padrão do passo, ou uma por disco/círculo — CEGO), `orientacao?` (`[x,y,z]`, direção do +u do anel) | FEITO (ciclo "Corte e orientação de seção v1", ampliado no A-30 e A-34). A primeira SUBTRAÇÃO do núcleo: abre furo CILÍNDRICO numa face PLANA e CONVEXA, pelo eixo `-N` da entrada. Em `lados:{desvio}`, deriva a menor contagem pela flecha do MAIOR raio do passo; a tolerância fica comparável entre peças, mas mudar o raio pode renumerar dentro do bloco. **Não é uma booleana genérica:** toca SÓ as faces nomeadas, toda face criada nasce endereçável e toda face destruída fica registrada. `{op:'furo',id}` é o furo inteiro; `borda`, `parede`, `saida`, `furo`, `grupo` e a tampa `'fundo'` recortam as famílias. Face não-plana/côncava, anel fora ou sobreposto, modo/medida inválidos e estouro derivado GRITAM e abortam o passo inteiro. |
...
preserva a numeração uniforme das famílias; na forma `{desvio}`, ele é derivado
do maior raio entre os grupos.
